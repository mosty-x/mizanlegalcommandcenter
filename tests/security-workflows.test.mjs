...
test("the guide contains all five workflows and a workspace launch gate",async()=>{const guide=await read("components/onboarding-gate.tsx");const tools=await read("lib/tool-definitions.ts");for(const slug of ["enforceability","disputes","deal-room","regulatory","client-command"])assert.match(tools,new RegExp(`slug: "${slug}"`));assert.match(guide,/viewedWorkflows/);assert.match(guide,/acceptedTerms/);});
