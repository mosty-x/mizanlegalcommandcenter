...
export function ToolWorkspace({ toolSlug }: { toolSlug: ToolDefinition["slug"] }) {
  ...
  const [specialization,setSpecialization]=useState<{label:string;revision:number}|null>(null);
  ...
  const [framework,setFramework]=useState("EG");
  const [clientSafeDocumentsConfirmed,setClientSafe]=useState(false);
  ...
  const loadData = useCallback(async () => {
    const [providerResponse, documentResponse, runResponse,profileResponse] = await Promise.all([
      fetch("/api/providers", { cache: "no-store" }),
      fetch(`/api/documents?tool=${tool.slug}`, { cache: "no-store" }),
      fetch(`/api/runs?tool=${tool.slug}`, { cache: "no-store" }),
      fetch(`/api/specialization?tool=${tool.slug}`, { cache: "no-store" }),
    ]);
    ...
    if(profileResponse.ok){const p=(await profileResponse.json()).snapshot;setSpecialization({label:p.profile.label,revision:p.revision});}
  ...
  async function runWorkflow() {
    ...
    try {
      const response = await fetch("/api/workflows/run", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ toolSlug: tool.slug, providerId, documentIds: selected, title, objective, jurisdiction,asOf,framework,clientSafeDocumentsConfirmed }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "التشغيل ماكملش.");
      if(data.run.output)setTrace(data.run);
      else setMessage(`التشغيل ماكملش. ${(data.run.nodes??[]).filter((n:{status:string})=>n.status==="failed").map((n:{label:string})=>n.label).join("، ")}`);
      await loadData();
  ...
  async function openRun(id: string) {
    ...
    if (response.ok) {
      if(data.run.output)setTrace(data.run);
      else setMessage(`التشغيل غير مكتمل: ${(data.run.nodes??[]).filter((n:{status:string})=>n.status==="failed").map((n:{label:string})=>n.label).join("، ")}`);
      requestAnimationFrame(() => document.getElementById("workflow-result")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  ...
  return (
    <div className={`page-wrap tool-page accent-${tool.accent}`}>
      ...
      {specialization&&<p className="tool-profile-note">التخصص الحالي: {specialization.label} · الإصدار {specialization.revision} <Link href="/specialize">راجع تعليمات التخصص</Link></p>}
      ...
      <div className="workspace-grid">
        ...
        <section className="workspace-card run-panel">
          ...
          <div className="field-pair">
            <label className="field-label"><span>الاختصاص</span><Input value={jurisdiction} onChange={()=>setJurisdiction("مصر")} readOnly /></label>
          ...
          <div className="field-pair"><label className="field-label"><span>التاريخ المطلوب بحث القانون عنده</span><Input type="date" value={asOf} onChange={e=>setAsOf(e.target.value)} max={new Date().toISOString().slice(0,10)}/></label><label className="field-label"><span>قواعد التحكيم المتفق عليها</span><PremiumSelect value={framework} onValueChange={setFramework} ariaLabel="قواعد التحكيم" placeholder="اختار قواعد التحكيم" options={[{value:"EG",label:"لم تُحدد قواعد مؤسسية"},{value:"CRCICA-2024",label:"قواعد مركز القاهرة ٢٠٢٤"},{value:"CRCICA-2011",label:"قواعد مركز القاهرة ٢٠١١"}]}/></label></div>
          <Link href="/knowledge" className="secondary-action">راجع الذاكرة القانونية والمصادر المسموحة</Link>
          {tool.slug==="client-command"&&<label className="memory-consent"><input type="checkbox" checked={clientSafeDocumentsConfirmed} onChange={e=>setClientSafe(e.target.checked)}/><span>راجعت المستندات المحددة وهي صالحة لاستخدامها في موجز العميل.</span></label>}
          <div className="execution-box">
            <div><ShieldCheck /><span><strong>قبل ما تبدأ</strong><small>{selectedProvider?.provider === "ssh-gateway" ? "المقتطفات والنصوص القانونية المسموحة هتتبعت لسيرفرك لتشغيل المسار المصري الكامل. يحتاج مستقبل النسخة الثانية." : "المقتطفات هتُحلل بالخدمات المحددة، والسند القانوني هييجي من الذاكرة المراجعة. النتيجة غير المسندة هتتحجب."}</small></span></div>
