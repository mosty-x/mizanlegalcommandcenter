...
async function readLimitedResponse(response: Response): Promise<unknown> {
  ...
  try {
    while (true) {
      ...
      if (size > 1_500_000) { await reader.cancel(); throw new Error("PROVIDER_RESPONSE_TOO_LARGE"); }
...
export async function callProvider(args: {
  ...
  const signal = args.signal ? AbortSignal.any([controller.signal, args.signal]) : controller.signal;
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        model: config.model,
        ...(args.cacheSalt && config.provider === "openai-compatible" ? { cache_salt: args.cacheSalt } : {}),
        temperature: 0.1,
        max_tokens: maxOutputTokens,
