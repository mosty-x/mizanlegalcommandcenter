...
export async function sshExchange(args: {
  ...
  const maxTimeout = (args.packet.workflow as {version?:string}|undefined)?.version === "egypt-rag-2.1.0" ? 240000 : 90000;
  const timeout = setTimeout(() => controller.abort(), Math.min(args.timeoutMs ?? 90000, maxTimeout));
