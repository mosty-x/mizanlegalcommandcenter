...
export async function parseJsonWithLimit(request: Request, maxBytes = 256_000): Promise<unknown> {
  ...
  if(!reader)throw new Error("PAYLOAD_TOO_LARGE");
  const chunks:Uint8Array[]=[];let size=0;
  try { while(true){const {done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>maxBytes)throw new Error("PAYLOAD_TOO_LARGE");chunks.push(value);} }
...
export function safeErrorCode(error: unknown): string {
  ...
  const allowed = new Set([
    "RAG_REQUIRED_AUTHORITY_MISSING", "RAG_REQUIRED_CONTEXT_CAP", "RAG_PROFILE_CONFLICT", "RAG_PROFILE_NOT_FOUND", "RAG_PROFILE_LIMIT", "RAG_CONTEXT_BUDGET",
    "RAG_SOURCE_DENIED", "RAG_POLICY_REQUIRED", "RAG_DATE_INVALID", "RAG_SOURCE_CONFLICT",
    "RAG_CAPABILITY_MISSING", "RAG_EMBEDDING_INVALID", "RAG_OUTPUT_INVALID", "RAG_SERVICE_FAILED",
    "RAG_BUDGET_EXCEEDED", "RAG_CORPUS_LIMIT", "RAG_IMPORT_INVALID", "RAG_IMPORT_CONFLICT",
    "RAG_APPROVAL_DENIED", "RAG_DOCUMENT_SCOPE", "RAG_CLIENT_APPROVAL_REQUIRED", "RAG_INPUT_TOO_LARGE", "RAG_BUSY", "RAG_DAG_INVALID", "RAG_CORPUS_CHANGED", "RAG_SSH_VERSION_REQUIRED",
